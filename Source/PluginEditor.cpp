...
#if __has_include("../build/alletis-fluorescence/vs2026/juce_binarydata_AlletisFluorescenceData/JuceLibraryCode/BinaryData.h")
 #include "../build/alletis-fluorescence/vs2026/juce_binarydata_AlletisFluorescenceData/JuceLibraryCode/BinaryData.h"
#elif __has_include("BinaryData.h")
 #include "BinaryData.h"
#else
 #error "BinaryData.h not found. Run CMake configure/build to generate AlletisFluorescenceData."
#endif
#include "SpectrumDisplay.h"
FluorescenceAudioProcessorEditor::FluorescenceAudioProcessorEditor(FluorescenceAudioProcessor& p)
      ...
      scalePanel(p.apvts)
{
    ...
    hoverHintBox.setOnHeightChanged([safe = juce::Component::SafePointer<FluorescenceAudioProcessorEditor> (this)]
    {
        if(safe != nullptr)
            safe->resized();
    });
...
void FluorescenceAudioProcessorEditor::applyModeAlphas()
{
...
void FluorescenceAudioProcessorEditor::timerCallback()
{
...
void FluorescenceAudioProcessorEditor::setupKnob(ValueKnob& k, const juce::String& id,
                                                std::unique_ptr<SA>& att)
{
    content.addAndMakeVisible(k);
    if(id == "pitch" || id == "scPitch")
        k.setSnapStep(1.0);
    else if(id == "fineTune")
        k.setSnapStep(25.0);
...
void FluorescenceAudioProcessorEditor::wireReadout(ValueKnob& k, juce::Label& lbl)
{
...
void FluorescenceAudioProcessorEditor::showReadout(ValueKnob* k, juce::Label* lbl, ReadoutMode mode)
{
    readoutKnob = k;
    readoutLabel = lbl;
    readoutMode = mode;
...
void FluorescenceAudioProcessorEditor::scheduleHoverReadoutShow(ValueKnob* knob)
{
    const int token = ++readoutTimerToken;
    juce::Timer::callAfterDelay(250, [safe = juce::Component::SafePointer<FluorescenceAudioProcessorEditor> (this), token, knob]
    {
        if(safe == nullptr)
            return;
...
void FluorescenceAudioProcessorEditor::updateReadout()
{
...
void FluorescenceAudioProcessorEditor::hideReadout()
{
    readoutKnob = nullptr;
...
void FluorescenceAudioProcessorEditor::rebuildLogo()
{
...
void FluorescenceAudioProcessorEditor::styleLabel(juce::Label& l, const juce::String& text)
{
...
void FluorescenceAudioProcessorEditor::MouseSpy::mouseDown(const juce::MouseEvent& e)
{
    owner.handleGlobalMouseDown(e);
}
void FluorescenceAudioProcessorEditor::MouseSpy::mouseMove(const juce::MouseEvent& e)
{
    owner.handleGlobalMouseMove(e);
}
juce::Label* FluorescenceAudioProcessorEditor::findReadoutLabelForKnob(ValueKnob* knob)
{
...
void FluorescenceAudioProcessorEditor::handleGlobalMouseMove(const juce::MouseEvent& e)
{
...
juce::String FluorescenceAudioProcessorEditor::findHoverHintTextForComponent(juce::Component* eventComponent) const
{
    ...
    for(auto* c = eventComponent; c != nullptr; c = c->getParentComponent())
    {
        ...
        if(c == &hysteresisBtn) return "Hysteresis\n\nStabilises fundamental detection over time. \nFundamentals with high confidence persist through brief dropouts and resist flicker between nearby target pitches.";
...
void FluorescenceAudioProcessorEditor::handleGlobalMouseDown(const juce::MouseEvent& e)
{
...
void FluorescenceAudioProcessorEditor::paint(juce::Graphics& g)
{
    g.fillAll(juce::Colour(0xff121216));
}
void FluorescenceAudioProcessorEditor::ContentComponent::paint(juce::Graphics& g)
{
    g.fillAll(juce::Colour(0xff121216));
...
void FluorescenceAudioProcessorEditor::applySoftwareRenderer()
{
   #if JUCE_WINDOWS
...
void FluorescenceAudioProcessorEditor::parentHierarchyChanged()
{
    applySoftwareRenderer();
}
void FluorescenceAudioProcessorEditor::visibilityChanged()
{
    if(isShowing())
        applySoftwareRenderer();
}
void FluorescenceAudioProcessorEditor::resized()
{
