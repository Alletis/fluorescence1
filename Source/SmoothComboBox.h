...
public:
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        const float uiScale = b.getHeight() > 0.0f ? b.getHeight() / 24.0f : 1.0f;
        const float aw = 9.0f * uiScale, ah = 4.5f * uiScale;
        const float ax = b.getRight() - 13.0f * uiScale, ay = b.getCentreY();
        juce::Path p;
        ...
        const auto textBounds = getLocalBounds().reduced(juce::roundToInt(6.0f * uiScale), juce::roundToInt(2.0f * uiScale))
                            .withTrimmedRight(juce::roundToInt(18.0f * uiScale));
        const auto textCol = isEnabled() ? juce::Colour(0xffe8e8ee) : juce::Colour(0xff55555c);
        g.setColour(textCol);
        g.setFont(KnobLookAndFeel::courier(juce::jlimit(8.0f, 40.0f, b.getHeight() * 0.50f)));
