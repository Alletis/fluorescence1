...
public:
    ...
    void mouseDrag(const juce::MouseEvent& e) override
    {
        ...
        if(e.mods.isShiftDown() && snapStep > 0.0)
        {
            const double snapped = juce::jlimit(getMinimum(), getMaximum(),
                                                getMinimum() + std::round((getValue() - getMinimum()) / snapStep) * snapStep);
            if(std::abs(snapped - getValue()) > 1.0e-6)
    ...
    void setSnapStep(double step) { snapStep = step > 0.0 ? step : 0.0; }
