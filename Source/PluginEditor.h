...
class FluorescenceAudioProcessorEditor : public juce::AudioProcessorEditor,
                                       private juce::Timer
{
public:
    explicit FluorescenceAudioProcessorEditor(FluorescenceAudioProcessor&);
    ~FluorescenceAudioProcessorEditor() override;
...
private:
    ...
    FluorescenceAudioProcessor& proc;
    KnobLookAndFeel knobLnf;
    struct ContentComponent : juce::Component
    {
        FluorescenceAudioProcessorEditor& owner;
        explicit ContentComponent(FluorescenceAudioProcessorEditor& o) : owner(o) {}
        void paint(juce::Graphics&) override;
    };
    ...
    {
        FluorescenceAudioProcessorEditor& owner;
        explicit MouseSpy(FluorescenceAudioProcessorEditor& o) : owner(o) {}
        void mouseDown(const juce::MouseEvent& e) override;
        void mouseMove(const juce::MouseEvent& e) override;
    };
                     ...
                     private juce::Timer
    {
        ...
        {
            const float nextTarget = b ? 1.0f : 0.0f;
        ...
        void primeTo(bool b)
        {
            target = amount = b ? 1.0f : 0.0f;
            start = amount;
            phase = 1.0f;
            animating = false;
            setVisible(amount > 0.0f);
        }
        void paint(juce::Graphics& g) override
        {
    ...
    private:
        void timerCallback() override
        {
            ...
            phase = juce::jmin(1.0f, phase + phaseStep);
            const float eased = 0.5f - 0.5f * std::cos(phase * juce::MathConstants<float>::pi);
            amount = start + (target - start) * eased;
            setVisible(true);
            toFront(false);
            repaint();
            if(phase >= 1.0f)
        ...
        static constexpr float phaseStep = 1.0f / 12.0f;
        float amount = 0.0f, start = 0.0f, target = 0.0f, phase = 1.0f;
                        ...
                        private juce::Timer
    {
        ...
        void paint(juce::Graphics& g) override
        {
                ...
                g.drawImage(logoImg, logoRect.toFloat());
            {
                ...
                g.setFont(KnobLookAndFeel::courier(KnobLookAndFeel::uiFont));
                g.drawText("           Version 1.1.6", row, juce::Justification::centredLeft, false);
            ...
            if(page == 0)
            {
                ...
                drawSection(g, "WITH INSPIRATION FROM", y, pageAlpha); y += 26;
            ...
            else
            {
                ...
                y += 14;
                for(auto* n : { "Cure Audio, DTMWiki Community, ZL Audio;",
                                "A.C.F., bpmstall, Ember, EndBlue, farah,",
                                "Fogsy, Hck, IAMMRGODIE, Killy, KPa_11,",
                                "Meowtronix, NapKid, Noya, NULLCOL0R,",
                                "patient_sHadow, Rainbow Illusion, RHYX,",
                                "saundix, sout, sukabing, YuanYuy;"})
                {
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(FluorescenceAudioProcessorEditor)
};