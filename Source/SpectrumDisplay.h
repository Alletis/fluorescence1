...
public:
    ...
    SpectrumDisplay(juce::AudioProcessorValueTreeState& state,
                     FluorescenceAudioProcessor::SpectrumBridgeType& bridgeIn)
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        g.fillRoundedRectangle(plot, spectrumCornerRadius);
        {
            ...
            if(disableUiEnabled)
            {
                const float shadeAlpha = 1.0f - bypassVisualAmt;
                ...
                g.setColour(juce::Colour(0xff000000).withAlpha(0.55f * shadeAlpha));
                ...
                juce::ColourGradient enabledGrad(juce::Colour(0xff2b2b33).withAlpha(0.44f * shadeAlpha),
                                                  enabledBand.getCentreX(), plotY,
                                                  juce::Colour(0xff1e1e24).withAlpha(0.34f * shadeAlpha),
...
private:
    ...
    using Snapshot = FluorescenceAudioProcessor::SpectrumBridgeType::Snapshot;
    ...
    void timerCallback() override
    {
        ...
        {
            bypassVisualAmt = bypassVisualStart = bypassVisualTarget = tgtBypass;
            bypassVisualPhase = 1.0f;
            bypassVisualPrimed = true;
        }
        else if(tgtBypass != bypassVisualTarget)
        {
            bypassVisualStart = bypassVisualAmt;
            bypassVisualTarget = tgtBypass;
            bypassVisualPhase = 0.0f;
        }
        if(bypassVisualPhase < 1.0f)
        {
            bypassVisualPhase = juce::jmin(1.0f, bypassVisualPhase + bypassVisualPhaseStep);
            const float eased = 0.5f - 0.5f
                              * std::cos(bypassVisualPhase * juce::MathConstants<float>::pi);
            bypassVisualAmt = bypassVisualStart
                            + (bypassVisualTarget - bypassVisualStart) * eased;
        }
    ...
    FluorescenceAudioProcessor::SpectrumBridgeType& bridge;
    ...
    float bypassVisualStart = 0.0f;
    float bypassVisualTarget = 0.0f;
    float bypassVisualPhase = 1.0f;
    static constexpr float bypassVisualPhaseStep = 1.0f / 12.0f;
