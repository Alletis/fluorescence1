...
FluorescenceAudioProcessor::FluorescenceAudioProcessor()
...
void FluorescenceAudioProcessor::handleAsyncUpdate()
{
    setLatencySamples(pendingLatency.load());
}
juce::AudioProcessorValueTreeState::ParameterLayout
FluorescenceAudioProcessor::createLayout()
{
...
const juce::String FluorescenceAudioProcessor::getName() const { return "Fluorescence"; }
bool FluorescenceAudioProcessor::acceptsMidi() const { return true; }
bool FluorescenceAudioProcessor::producesMidi() const { return false; }
bool FluorescenceAudioProcessor::isMidiEffect() const { return false; }
double FluorescenceAudioProcessor::getTailLengthSeconds() const
{
...
int FluorescenceAudioProcessor::getNumPrograms() { return 1; }
int FluorescenceAudioProcessor::getCurrentProgram() { return 0; }
void FluorescenceAudioProcessor::setCurrentProgram(int) {}
const juce::String FluorescenceAudioProcessor::getProgramName(int) { return {}; }
void FluorescenceAudioProcessor::changeProgramName(int, const juce::String&) {}
void FluorescenceAudioProcessor::prepareToPlay(double sr, int)
{
    sampleRate = sr;
...
void FluorescenceAudioProcessor::releaseResources() { }
bool FluorescenceAudioProcessor::isBusesLayoutSupported(const BusesLayout& layouts) const
{
...
void FluorescenceAudioProcessor::reconfigure(int newOrder, int newOverlap)
{
...
void FluorescenceAudioProcessor::buildSynthesisKernel()
{
    const int N = fftSize;
...
float FluorescenceAudioProcessor::analyze(int channel)
{
...
float FluorescenceAudioProcessor::detectTransient(int channel)
{
    float* prevLog = prevLogMag[(size_t) channel].data();
    auto& prevMag = prevPvMag[(size_t) channel];
    auto& bypassRe = transientBypassRe[(size_t) channel];
...
float FluorescenceAudioProcessor::effectiveDisableFreqHi() const noexcept
{
...
bool FluorescenceAudioProcessor::isDisableLoActive() const noexcept
{
    return disableActiveLoParam == nullptr || disableActiveLoParam->load() >= 0.5f;
}
bool FluorescenceAudioProcessor::isDisableHiActive() const noexcept
{
    return disableActiveHiParam == nullptr || disableActiveHiParam->load() >= 0.5f;
}
void FluorescenceAudioProcessor::applyFormantShift(int channel)
{
...
float FluorescenceAudioProcessor::salienceIdx(int candPeak, int harmHorizon) const
{
...
float FluorescenceAudioProcessor::nearestTargetMidi(float baseHz) const
{
...
float FluorescenceAudioProcessor::snapToTargetMidi(float baseHz, float targetMidi, float amount) const
{
    if(amount <= 0.0f || baseHz <= 0.0f || targetMidi < -900.0f) return baseHz;
    const float midi = 69.0f + 12.0f * std::log2 (baseHz / 440.0f);
    return baseHz * std::pow(2.0f, (targetMidi - midi) / 12.0f * amount);
}
float FluorescenceAudioProcessor::snapBaseToScale(float baseHz, float amount, float fineCents) const
{
...
float FluorescenceAudioProcessor::membershipSigma(int harmonic, float freqHz) const
{
...
void FluorescenceAudioProcessor::updateBaseTracker(int channel, float frameTonal)
{
...
bool FluorescenceAudioProcessor::resolvePeakObservation(int channel, int sourceBin,
                                                        int& analysisBin,
                                                        float& peakHz) const
{
...
void FluorescenceAudioProcessor::rebuildHarmonicPeakList(int channel, float frameMaxMag)
{
    const float nyquist = (float) (sampleRate * 0.5);
    const float transformedBinWidth = juce::jmax(1.0e-6f, binWidth * pitchRatio);
...
void FluorescenceAudioProcessor::updateDetectionFrameParams()
{
...
void FluorescenceAudioProcessor::detect(int channel)
{
    ...
    base_detection::Config discoveryConfig;
    discoveryConfig.sampleRate = sampleRate;
    discoveryConfig.binWidth = binWidth;
    discoveryConfig.pitchRatio = pitchRatio;
    discoveryConfig.fMin = fMin;
    discoveryConfig.fMax = fMax;
    discoveryConfig.centsTolerance = centsTolerance;
    discoveryConfig.gatedThreshold = gatedThr;
    discoveryConfig.previousPrimary = prevPrimary[(size_t) channel];
    discoveryConfig.peakLimit = frame.peakLimit;
    discoveryConfig.baseLimit = baseLimit;
    discoveryConfig.detectionPeakCapacity = maxDetectionPeaks;
    discoveryConfig.rawPeakLimit = maxPeaks;
    const auto discovery = base_detection::discover(
        pvMag.data(), numBins, discoveryConfig,
        [this, channel] (int sourceBin, int& analysisBin, float& frequency)
        {
            return resolvePeakObservation(channel, sourceBin, analysisBin, frequency);
        },
        peakBin, peakFreq, peakAmp, peakAmpWork, harmonicMap, rawPeaks, baseFreq, baseSal);
    numPeaks = discovery.numPeaks;
    numBases = discovery.numBases;
    const float maxMag = discovery.maxMagnitude;
    if(numPeaks == 0) return;
    for(int i = 0; i < numPeaks; ++i)
    {
        peakBaseIdx[(size_t) i] = -1;
        peakHarmonic[(size_t) i] = 0;
        peakRatio[(size_t) i] = 1.0f;
    }
    const float totalPeak = discovery.totalPeak;
    const float firstSal = discovery.firstSalience;
    ...
    for(int i = 0; i < numPeaks; ++i)
    {
        ...
        if(w < 1.0e-3f)
            continue;
        const float appliedW = w + snapMix * (1.0f - w);
        ...
        const float dest = fk0 * std::pow(corrected / fk0, appliedW);
    ...
    for(int k = 0; k < numBins; ++k)
    {
        ...
        if(lastBase >= 0)
        {
            ...
            if(inv > 0.0f)
            {
                ...
                const float r = q * invNN;
                const float d = (r >= 1.0f) ? r : 1.0f / r;
                if(d < 1.0015f) { bBase = lastBase; bN = nn; bestRatioDist = d; }
            }
        }
        if(bBase < 0)
            for(int b = 0; b < numBases; ++b)
        ...
        const float appliedWb = wb + snapMix * (1.0f - wb);
...
float FluorescenceAudioProcessor::harmonicAssignmentTolerance(int harmonic,
                                                               float freqHz) const
{
    return membershipSigma(juce::jmax(1, harmonic), juce::jmax(1.0f, freqHz));
}
void FluorescenceAudioProcessor::rebuildFlatBaseCandidates()
{
...
bool FluorescenceAudioProcessor::findBaseForHarmonic(float partialHz, int harmonic,
...
void FluorescenceAudioProcessor::updatePartialHarmonicAssignment(Partial& partial,
                                                                int peakIndex,
                                                                bool newPartial)
{
    int candidateBase = -1;
...
void FluorescenceAudioProcessor::updatePartials(int channel)
{
...
void FluorescenceAudioProcessor::identifyHarmonic(float freqAnalysis,
...
void FluorescenceAudioProcessor::processPV(int channel)
{
    detect(channel);
}
void FluorescenceAudioProcessor::mapToDestination(int channel)
{
...
float FluorescenceAudioProcessor::envelopeCompensationGainAt(float frequencyHz) const noexcept
{
...
void FluorescenceAudioProcessor::applyTimeAdditiveCompensation(int channel)
{
...
bool FluorescenceAudioProcessor::isTimeAdditiveEnabled() const noexcept
{
    return tonalRendererParam != nullptr
        && juce::roundToInt(tonalRendererParam->load()) == 1;
}
float FluorescenceAudioProcessor::estimateTimeAdditiveAmplitude(int peakIndex) const
{
...
void FluorescenceAudioProcessor::updateTimeAdditiveVoices(int channel)
{
...
void FluorescenceAudioProcessor::removeTimeAdditiveOwnedSpectrum(int channel)
{
...
void FluorescenceAudioProcessor::renderTimeAdditive(int channel)
{
    auto& voices = timeVoices[(size_t) channel];
...
void FluorescenceAudioProcessor::synthesizeAdditive(int channel, bool reseed)
{
    float* fd = fftData.data();
    ...
    for(int j = 0; j < numBins; ++j)
    {
        ...
        if(bypassMask[(size_t) j] != 0)
        {
            outPh[j] = pvPhase[(size_t) j];
            if(! isMutedDisableMask(bypassMask[(size_t) j]))
            {
...
void FluorescenceAudioProcessor::captureSpectrum(int channel)
{
    ...
    if(pvBypassed)
    {
        for(int j = 0; j < nb; ++j)
        {
            const float re = fftData[(size_t) (2 * j)];
            const float im = fftData[(size_t) (2 * j + 1)];
            outMag [(size_t) j] = std::sqrt(re * re + im * im) * spectrumNorm;
            outFreq[(size_t) j] = pvFreq[(size_t) j];
        }
    }
    else
    {
        const auto& bMask = transientBypassMask[(size_t) channel];
        const auto& bRe = transientBypassRe[(size_t) channel];
        const auto& bIm = transientBypassIm[(size_t) channel];
        for(int j = 0; j < nb; ++j)
        {
...
void FluorescenceAudioProcessor::resetProcessingStateForStereoModeSwitch()
{
...
void FluorescenceAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer,
                                             juce::MidiBuffer& midiMessages)
{
    ...
    if(scRun)
    {
        ...
        const float emphasis = juce::jlimit(0.0f, 1.0f, emphasisParam->load());
        scP.centsTolerance = centsTol + emphasis * emphasis * (centsTolMax - centsTol);
        scP.moreBases = (moreBasesParam != nullptr && moreBasesParam->load() >= 0.5f);
        scP.hysteresis = (hysteresisParam != nullptr && hysteresisParam->load() >= 0.5f);
...
bool FluorescenceAudioProcessor::hasEditor() const { return true; }
juce::AudioProcessorEditor* FluorescenceAudioProcessor::createEditor()
{
    return new FluorescenceAudioProcessorEditor(*this);
}
void FluorescenceAudioProcessor::getStateInformation(juce::MemoryBlock& destData)
{
...
void FluorescenceAudioProcessor::setStateInformation(const void* data, int sizeInBytes)
{
...
{
    return new FluorescenceAudioProcessor();
}